...
def get_chembl_to_label_map(h5_filepath='../data/raw/egfr_erbB1.h5'):
...
def extract_fp_and_labels():
    filepath = '../data/egfr_erbB1_morgan_2048.json'
    with open(filepath, 'r') as file:
        data = json.load(file)

    bit_infos = {item['chembl_id'] : item['bit_info'] for item in data['molecules']}
    morgan_fp = [item['morgan'] for item in data['molecules']]
    ...
    with open('../data/bit_info.json', 'w') as f:
        json.dump(bit_infos, f)
